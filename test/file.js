...
describe('file: getUrl',function(){
  ...
  it('with domain——domain是对象，且键是图片，对图片的通用处理',function(){
    ...
    path = __dirname+'/file/css/test.less?__inline';
    f = _.wrap(path);
    f.useHash = false;
    url = f.getUrl(false,true);
    //同级目录下没有css文件，所以不会被命中
    expect(url).to.equal('/file/css/test.less?__inline');
  });
...
describe('file: addRequire(id)',function(){
  ...
  it('general',function(){
    //第一次加
    var f = _.wrap(path);
    f.addRequire(__dirname+'/file/css/test.less');
    expect(f.requires).to.deep.equal([
      __dirname+'/file/css/test.less'
    ]);
    //重复添加依赖
    f.addRequire(__dirname+'/file/css/test.less');
    expect(f.requires).to.deep.equal([
...
describe('file: addAsyncRequire(id)',function(){
  ...
  it('general',function(){
    //第一次加
    var f = _.wrap(path);
    f.addAsyncRequire(__dirname+'/file/css/test.less');
    expect(f.asyncs).to.deep.equal([
      __dirname+'/file/css/test.less'
    ]);
    //重复添加依赖
  ...
  it('same path in requires', function() {
    var f = _.wrap(path);
    f.addRequire(__dirname+'/file/css/test.less');
    expect(f.addAsyncRequire(__dirname+'/file/css/test.less')).to.equal(__dirname+'/file/css/test.less');
    expect(f.requires).to.deep.equal([
      __dirname+'/file/css/test.less'
    ]);
    expect(f.asyncs).to.deep.equal([]);
