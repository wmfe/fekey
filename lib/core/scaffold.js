...
function analysic(params) {
    if (exists(config.templatePath)) {
        ...
        var defaultConfigReps = config.customRepo + params.initType;
        var initTypeConfig = initConfig['detail'][params.initType];
        if (initTypeConfig) {
...
function deploy(templatePath, scaffoldConf) {
    ...
    prompt({conf: conf})
        .then(function(params) {
            ...
            config.ignore = conf.ignore || config.ignore;
            var files = _.find(templatePath);
            files = filter(files);
...
function deployPath(params) {
    ...
    fis.util.map(params.files, function(index, path) {
        var pathTemp = path;
...
function after(params) {
    if (params.conf && params.conf.after) {
        // 将正则转换为字符串
        config.keyword_reg = regToStr(config.keyword_reg);
