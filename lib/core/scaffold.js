...
var config = {
    customFold: 'fekey-scaffold-template',
    templatePath: '',
...
scaffold.start = function(initType, targetPath) {
    ...
    if (fs.existsSync(config.templatePath)) {
        ...
        var repo = 'wmfe/fekey-scaffold-' + initType;
...
function deploy(templatePath, scaffoldConf) {
    ...
    // 输入数据
    scaffoldKernel.prompt(conf.prompt, function(err, results) {
        if (err) {
            fis.log.error(err);
        }
        results = postprocess({
...
function deployPath(params) {
    ...
    let pathReg = params.conf.pathReg || config.pathReg;
    // 将正则转换为字符串
    pathReg = regToStr(pathReg);
    fis.util.map(files, function(index, path) {
        var pathTemp = path;
        fis.util.map(params.results, function(key, value) {
            let reg = pathReg.replace('key', key);
            // 将字符串转化为正则
            reg = strToReg(reg);
    ...
    if (params.release) {
        ...
        fis.util.map(params.release, function(index, path) {
            if (fis.util.isDir(path.releasePath)) {
                ...
                let contentReg = params.conf.contentReg || config.contentReg;
                // 将正则转换为字符串
                contentReg = regToStr(contentReg);
                fis.util.map(params.results, function(key, value) {
                    let reg = contentReg.replace('key', key);
                    // 将字符串转化为正则
                    reg = strToReg(reg);
                    // 对原始文件内容进行模板数据替换
...
function after(params) {
    console.log(params.conf.after.join('\n'));
