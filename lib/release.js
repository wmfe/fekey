...
function buildPack(ret) {
  ...
  Object.keys(src).forEach(function(subpath) {
    var file = src[subpath];
    if (!file.autoSmartPack || file.release === false || file.isPartial) {
      return;
    }

    if(file.autoSmartPack) {
      var folderName = getFolderName(file.subpath);
      if(file.filename == folderName) {
          var requires = file.requires;
          var asyncs = file.asyncs;
          pack[file.subdirname + '/' + file.filename + '_pkg_deps.js'] = pack[file.subdirname + '/' + file.filename + '_pkg_deps.js'] || [];
          pack[file.subdirname + '/' + file.filename + '_pkg_asyncs.js'] = pack[file.subdirname + '/' + file.filename + '_pkg_asyncs.js'] || [];
          pack[file.subdirname + '/' + file.filename + '_pkg_deps.js'].push(file.subdirname + '/' + file.filename + '.js');
          pack[file.subdirname + '/' + file.filename + '_pkg_deps.js'].push(file.subdirname + '/' + file.filename + '.js' + ':deps');
          pack[file.subdirname + '/' + file.filename + '_pkg_asyncs.js'].push(file.subdirname + '/' + file.filename + '.js' + ':asyncs');
      }
    }
  });
  return pack;
...
function getFolderName(subpath) {
    var reg = /\/[^\/]*\/[^\/]*\./;
    var result = reg.exec(subpath)[0];
    result = result.substring(result.indexOf('/') + 1, result.lastIndexOf('/'));
    return result;
}

function getExt(subpath) {
    var reg = /\..*/;
    var result = reg.exec(subpath)[0];
...
function getSubpath(namespacePath) {
    var result = namespacePath.substring(namespacePath.indexOf(':') + 1, namespacePath.length);
